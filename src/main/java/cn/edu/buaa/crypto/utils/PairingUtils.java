...
//    }
    ...
    public static SigPublicParameter SigKeyGen(PairingKeySerParameter publicKeyParameter) {
        Pairing pairing = PairingFactory.getPairing(publicKeyParameter.getParameters());

        Element lambda = pairing.getZr().newRandomElement().getImmutable();
        Element sig_g = pairing.getG1().newRandomElement().getImmutable();
        Element sig_B = sig_g.powZn(lambda).getImmutable();
        Element sig_Z = pairing.pairing(sig_g, sig_g).getImmutable();

        return new SigPublicParameter(sig_g, sig_B, sig_Z, lambda);
    }

    public static Element SigSign(PairingKeySerParameter publicKeyParameter, SigPublicParameter sig_pk, Element sig_sk, Element x) {
        Pairing pairing = PairingFactory.getPairing(publicKeyParameter.getParameters());

        Element temp = sig_sk.add(x).getImmutable();
        temp = temp.invert().getImmutable();
        Element sig = sig_pk.getG().powZn(temp).getImmutable();
        return sig;
    }

    public static boolean SigVerify(PairingKeySerParameter publicKeyParameter, SigPublicParameter sig_pk, Element sig, Element x) {
        Pairing pairing = PairingFactory.getPairing(publicKeyParameter.getParameters());

        Element temp = pairing.pairing(sig, sig_pk.getB().mul(sig_pk.getG().powZn(x))).getImmutable();
        return temp.isEqual(sig_pk.getZ());
    }
    ...
    public static boolean IsFather(String a, String b) //判断a是不是b的祖先节点
    {
        if (a == "E" && b != "E")
            return true;
    ...
    public static Map<String, PairingKeySerParameter> PunctureTree(PairingKeySerParameter publicKeyParameter, HIBEBBG05Engine hibebbg05Engine, Map<String, PairingKeySerParameter> tk, String n) {

        Map<String, PairingKeySerParameter> tkP = new HashMap<String, PairingKeySerParameter>();
                ...
                while (GetFather(ntmp) != null && nP.length() < ntmp.length()) {
                    ...
                    if (IsFather(nP, nodeString)) {
                        ...
                        if (!tkP.containsKey(nodeString)) {
                            PairingKeySerParameter delegateKey = hibebbg05Engine.delegate(publicKeyParameter, tk.get(nP), delegateString);
...
//        System.out.println("并集：" + result);
    ...
    public static Map<String, PairingKeySerParameter> GetElementMapFromBytes(Pairing pairing, byte[][] byteArrays) {
        Map<String, PairingKeySerParameter> map = new HashMap<String, PairingKeySerParameter>();
        for (int i = 0; i < byteArrays.length; i++) {
